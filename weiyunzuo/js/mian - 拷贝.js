...
function serviceInove(page){
	$(".leftArow").click(function(){
		...
		if(!$proUl.is(":animated")){
			...
			/*$(".ServiceMian").each(function() {
                $(this).find(".svicePosition .Smun").eq(page).find(".SerArt section:eq(1)").addClass("setOn");
				$(this).find(".svicePosition .Smun").eq(page).addClass("SmunOn");
            });*/
			}
		}
		})
