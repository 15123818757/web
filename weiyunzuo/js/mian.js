...
function mianWidth() {
	...
	var NumCaseHeight=$(".NumCase").height();
	$(".MunCase").css({
		"margin-top":(NumCaseHeight-550)/2
		});
...
function ServiceMian() {
    ...
    $(".ServiceNav ul li").click(function() {
        ...
        $(".ServiceMian").eq(index).removeClass("ServiceMHidden").find(".Smun").addClass("SmunOn");
		ServiceSeepen(page,index);
    });
    ServiceSeepen(page,index);
}

function ServiceSeepen(page,index){
    $(".leftArow").eq(index).click(function() {
	...
	$(".rightArow").eq(index).click(function() {
        $(".svicePosition .Smun").removeClass("SmunOn");
        $(".svicePosition .SerArt").find("section:eq(1)").removeClass("setOn");
        var i = 1;
        var $pro = $(this).parents(".ServiceMian"),
