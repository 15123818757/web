...
function fixAnim(b) {
	var videoStyle = document.getElementById("videoStyle");
	var t0 = 1000;
	if (b == -1) {
		...
		setTimeout(function() {
			...
			$(".insCle .insPn").delay(3000).addClass("insPnVisible").fadeIn("fast");
			$(".navUl li a").css({
				"color": "#fff"
			});
			$(".navUl li.on a").css({
			...
			$(".Ilogo img").attr({
				"src": "images/img/logoImg01.png"
			});
			$(".indexHeader menu li").css({
				"color": "#fff"
			});
			$(".indexHeader menu li:eq(1)").find("img").attr({
				"src": "images/icon/rwm01.png"
			});
		}, t0);
	} else {
		setTimeout(function() {
			...
			$(".Ilogo img").attr({
				"src": "images/img/logoImg.png"
			});
			$(".indexHeader menu li").css({
				"color": "#333"
			});
			$(".indexHeader menu li:eq(1)").find("img").attr({
				"src": "images/icon/rwm.png"
			});
		}, t0);
	}
	if (b == -2) {
	...
	if (b == -3) {
		$(".Smun").each(function() {
			var serIon = $(this).find(".SerArt section:eq(0) img").attr("data-urlImg");
			$(this).find(".SerArt section:eq(0) img").attr({
				"src": serIon
			});
		})
		setTimeout(function() {
	...
	if (b == -4) {
		$(".MunCase .NumCase_article").each(function() {
			var caseIon = $(this).find(".caseImages").attr("data-urlImg");
			$(this).find(".caseImages").attr({
				"src": caseIon
			});
		})
		$(".MunCase").hide();
